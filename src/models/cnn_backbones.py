...
def build_alexnet_fc7(freeze: bool = True) -> tuple[nn.Module, int]:
    ...
    AlexNet up to and including fc7 (classifier[5] = second ReLU).
    ...
    NOTE: eval() is NOT called here so that model.train() / model.eval()
    propagates correctly to the backbone (important when freeze=False).
    When freeze=True the parameters have requires_grad=False, which is
    sufficient to prevent weight updates without locking BN/Dropout mode.
    """
